...
| 优先级 | 目标 | 含义 |
|---|---|---|
| **P0** | 常驻内存最小化（✅ 2026-08 实测达成） | 服务端机器挤着一大堆服务，内存是稀缺资源。RSS 目标是原版（含 reqwest/rustls 的 ~30-50MB）的 1/3 到 1/2。**实测：真实量产稳态 4.3-5.2 MB / 峰值 4.6-5.4 MB（§10.1.1），低于 7-15MB 预算下界，已超额达成**——内存不再是紧约束，后续多功能（回放/反作弊/持久化）可放开做 |
| **P0** | 子系统可整体替换 | 房间管理、封禁、鉴权等任意子系统出现更好实现时，只换实现、不碰核心、不改其他模块 |
| **P1** | CPU 合理即可（实测：小包处理是真实瓶颈） | "不炸"就行，不做无意义的 CPU 极致优化。**注（2026-08 实测）**：回环 1500 连接高压下 load 飙到 29（2 核），RSS 却稳定——**真实瓶颈是每连接高频小包的 CPU 处理，非内存**（§10.1.1），若追求更高吞吐从 CPU 效率入手 |
...
- 不追求 CPU 极致——内存才是硬指标（**但 2026-08 实测后修正**：内存已达标，瓶颈转移到 CPU 小包处理，见 §1.2 P1 注）
...
- 分配器可选 mimalloc 减碎片
- 目标：几百连接常驻 RSS ~7-15MB（预算明细见 §10）——**2026-08 实测：真实量产稳态 ~4.3-5.2 MB，低于目标下界（实测明细见 §10.1.1）**
...
**本项目动作**：服务器默认跑 12346（监听 `0.0.0.0` IPv4——ISSUE-0008：Windows `[::]` 是 V6ONLY，IPv4 玩家连不上）；文档指引 SRV 配置；`phira-mp-client` 复用 resolver 做测活/测试工具。端口问题在"列表模型"下自然消失——它与灰度分流（§3.2）共用同一个**入口模型**：寻址 = 入口被找到的方式，灰度 = 入口被替换的方式。
...
**插座已装（2026-08）**：`Moderator` 已进 `phira-api`（§4.4 薄缝）——双方法 `intercept(cmd, ctx) -> Result<(), RoomError>` + `on_event(ev)`；拒绝码新增 `RoomErrorCode::Moderated` 且 `RoomErrorCode` 已 `#[non_exhaustive]`（§5.6：拒绝码集合开放演进，外部匹配必须带通配）。**bus 接入点**：`dispatch` 路由前调用 `intercept`——仅对 `Origin::Client` 命令（系统命令/生命周期事实/回注/配置热更不可被拦，core 保证），拒收的命令**不产生任何房间副作用**；`process_events` 完成后把**领域事件**（过滤热路径 `RelayTouches/Judges` 与 core 信号 `RoomClosed`）fire-and-forget 通知各观察者（不阻塞房间投递/串行位）。**测试位置 = phira-core 集成测试**（§4.9-3：observer 是 core 行为，契约套件直驱 actor 测不到）：拦截在路由前 / 系统命令不经过 / 领域事件过滤 三条已钉住。**接口仍按草案对待**：第一个真实观察者（谱面反作弊）动工时再定形——若需要"通知含 RoomClosed"或"按 user 过滤订阅面"，届时扩展，现在不承诺。
...
| HTTP 客户端（异步 + rustls 单栈） | ~2-3 MB（https 回源躲不掉；若官方 API 允许明文 HTTP 可省 ~1.5MB，但大概率不允许，按 https 计，评审 §6） |
| 房间/用户表（每房 8 人规模） | ~1-2 MB |
| **合计** | **~7-15 MB（实测见 §10.1.1：真实量产稳态 ~4.3-5.2 MB，低于预算下界）** |

### 10.1.1 实测数据（2026-08，真实生产 Linux + 回环高压）

> 预算表是设计期估算。2026-08 用**真实生产实例（Debian 12, 2 核, 2GB）+ 回环高压**做了实测，结果**大幅优于预算**。

**环境**：Debian GNU/Linux 12（bookworm）· 2 核 · 2GB RAM · `phira-server` 单二进制（release，未 strip）· `/proc` 读 `VmRSS`/`VmHWM`（Linux 标准，比 Windows 工作集更真实）。

| 场景 | 连接数 | 峰值 VmHWM | 稳态 VmRSS | 说明 |
|---|---|---|---|---|
| 留守（0 连接，监 3939 + 8080） | 0 | 5352 kB | 5336 kB | 启动后稳态基线 |
| **proto 灌流**（公网 47 Mbps） | 500 | 5400 kB | 5348 kB | 跨公网 71 MB 发送 |
| **mixed 对局**（玩家 + monitor） | 300 | 5424 kB | ~5378 kB | 完整对局全流程，p95 107ms |
| proto 灌流（公网） | 800 | 5424 kB | ~5366 kB | 高并发仍 ~5.2MB |
| **回环 Ping 高压**（本机，不走公网） | 1000 | 4604 kB | ~4490 kB | 合法协议，不刷 invalid 日志 |
| 回环 Ping 高压 | 1500 | 4720 kB | ~4360 kB | 无 panic、无泄漏 |

**关键结论（实测）**：

1. **稳态 RSS ≈ 4.3-5.2 MB**，低于预算下界（7 MB）——P0 内存目标**超额达成**，留出充足余量。
2. **内存几乎不随连接增长**：300→1500 连接，RSS 峰值仅 4.6-5.4 MB。§10.4 记账平衡 + 连接准入工作良好。
3. **热路径记账**：完整对局 + 多 monitor Touches 转播（EncodeCache 编码一次共享），`in_flight_bytes` 峰值仅 **6.1 MiB**（64 MiB 守卫的 ~10%），断开慢消费者后**回落至 0**（无泄漏，§10.4 成立）。
4. **真正瓶颈是 CPU（小包处理），不是内存**：回环 1500 连接高压，`load average` 飙到 **29**（2 核），但 RSS 稳定——事件驱动服务器处理大量小包的 CPU 开销是固有成本，与"内存最小化"设计取舍一致。
5. **健壮性**：1500 连接高压下无 panic、无崩溃、端口始终正常，压测后 CPU 自然回落。

> **诚实注记**：实测为 Windows 工作集（局部）与 Linux RSS 的**量级验证**，非严格 PSS 验收（§10.2 要求的 `cgroup memory.max` 24h 压测 + PSS 记录仍未做）。当前数据足以证明"7-15MB 预算充分"，但若要严格盖章 P0，仍建议补 Linux PSS 24h 压测 + CI `memory-check` job。
...
- **实测手段（2026-08 已验证）**：
  - 生产实例（Linux）：`/proc/<pid>/status` 读 `VmRSS`/`VmHWM`，压测过程每 0.5-1s 采样（见 `loopback_ping.py` 式采样）
  - **公网压测**：`flooder --mode proto|mixed` 从本地跨公网灌流（受公网带宽上限 ~47 Mbps 限制，测的是服务器处理但不满载）
  - **回环压测**（不走公网，测服务器真实极限）：实例本机发起，`flooder` 或脚本打 `127.0.0.1:3939`——**必须用合法协议帧**（如 Ping，tag 0），否则丢随机字节被 `invalid packet` 拒绝会刷爆日志（2026-08 实测教训：随机字节 5 分钟刷 5728 条 `connection from` 日志）
  - 热路径记账：集成测试量 `in_flight_bytes` 峰值（完整对局 + monitor Touches 转播，见 e2e `hotpath_touches_broadcast_memory`）
  - **区分内存瓶颈 vs CPU 瓶颈**：回环压测同时观察 `load average`——若连接数大增而 RSS 稳、load 飙高，则瓶颈是 CPU（小包处理）而非内存
...
| 阶段 4（按需） | 第一个观察者（封禁内存版）、轻量 HTTP 客户端、内存调优 | RSS 达标（~7-15MB，§10）——**2026-08 实测已达（§10.1.1）：稳态 4.3-5.2MB**，本阶段转向 CPU 效率（小包处理是真实瓶颈） |
