...
**总纪律：以代码为准。发现"文档说了、代码没有"→ 记入 `docs/issues/`（现有 0001-0012，0001-0006/0008/0009/0011 已解决，0007/0010/0012 待解决，格式见下）。**
...
> **当前未决**：ISSUE-0007（game_time 钩子——断线恢复缺"玩家进度"维度，移植形态见该 issue，低危预留）、ISSUE-0010（CreateRoom 非幂等——响应丢失重试得 RoomIdOccupied + 孤儿房风险，协议级限制，低危）、ISSUE-0012（SessionRegistry 只进不出——长寿命进程慢性增长，需拆原子表，低危）。
> **已解决（2026-08）**：ISSUE-0009（旧 TCP 命令无 epoch 校验——命令侧 epoch 校验 + stale_connection 回归测试）、ISSUE-0011（EncodeCache 指针键 ABA——方案 A 条目钉住源 Arc + 4 回归测试），各见文末修复记录。
