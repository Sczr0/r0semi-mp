...
> **当前未决**：ISSUE-0007（game_time 钩子——断线恢复缺"玩家进度"维度，移植形态见该 issue，低危预留）、ISSUE-0010（CreateRoom 非幂等——响应丢失重试得 RoomIdOccupied + 孤儿房风险，协议级限制，低危）。ISSUE-0009（旧 TCP 命令无 epoch 校验）**已解决（2026-08）**——命令侧 epoch 校验 + stale_connection 回归测试，见该 issue 文末修复记录。
